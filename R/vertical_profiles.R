			 ...
			 override_positive = FALSE)
{
  ...
  if ((geolocation[1,1]<geolocation[2,1]) |
      ((geolocation[1,1]==geolocation[2,1])&(geolocation[1,2]<geolocation[2,2]))) {
     latlon_dir <- 1
  } else {
     latlon_dir <- -1
  }
  ...
  crossref <- NULL
  #browser()
  for (i in 1:(dim(geolocation)[1]-1)) {
     print(paste('transect section', i, 'of', dim(geolocation)[1]-1))
     first_point <- (i==1)
     li <- find_intersections(geolocation[i:(i+1),], x_grid, y_grid, latitude, longitude, first_point)
     #print(li[[1]])
     #print(li[[2]])
	   if (first_point) {
       location_ll <- li[[1]]
  ...
  geolocation <- location_ll

  if (dim(geolocation)[1]==0) stop("The line segment given does not intersect any model cells on this grid.")
  i <- dim(geolocation)[1]
  ...
  if (robust) {
    mydata <- get_ereefs_profile(var_names=var_names, geolocation=as.numeric(geolocation[1,c('latitude','longitude')]),
    ...
    for (i in 2:length(eta)) {
      ...
      mydata <- get_ereefs_profile(var_names=var_names, geolocation=as.numeric(geolocation[i,c('latitude','longitude')]),
		     start_date = target_date, end_date = target_date, 
  		     input_file = input_file, input_grid = input_grid, eta_stem = eta_stem, override_positive=override_positive)
    ...
    values <- array(NA, dim=c(length(z_grid)-1, dim(geolocation)[1], length(var_names)))
  ...
  if ((geolocation[1,1]<geolocation[length(geolocation$latitude),1]) |
      ((geolocation[1,1]==geolocation[length(geolocation$latitude),1])&(geolocation[1,2]<geolocation[length(geolocation$latitude),2]))) {
     directn <- 1
  } else {
     directn <- -1
  }
  if (latlon_dir == directn) {
     ...
     eta <- eta[ind]
     botz <- botz[ind]
     geolocation <- geolocation[seq(dim(geolocation)[1], 1, -1),]
     location_edges <- location_edges[seq(dim(location_edges)[1], 1, -1),]
     values[,1:length(eta),] <- values[,ind,]
  }

  return_list <- list(eta=eta, botz=botz, z_grid=z_grid, values=values, cell_centres=geolocation, cell_intersections=location_edges, crossref = crossref)
}
			 ...
			 override_positive=FALSE)
{
  ...
  if (is.integer(geolocation)) {
     location_grid <- geolocation
  } else { 
    # Find the nearest grid-points to the sampling location
    grid_ind <- (latitude - geolocation[1])^2 + (longitude - geolocation[2])^2 
...
ereefs_nearest_cells <- function(points, spatial_grid) {
  ...
  idx <- vapply(
    seq_len(nrow(points)),
    function(row_id) {
      which.min(earth.dist(
        points$longitude[[row_id]],
        points$latitude[[row_id]],
  ...
  dplyr::bind_cols(
    points %>% dplyr::rename(request_latitude = latitude, request_longitude = longitude),
    spatial_grid[idx, , drop = FALSE]
  )
}

ereefs_densify_path <- function(geolocation, samples_per_segment = 200) {
  geolocation <- dplyr::as_tibble(geolocation)
  out <- vector("list", max(1, nrow(geolocation) - 1))
  for (i in seq_len(nrow(geolocation) - 1)) {
    fraction <- seq(0, 1, length.out = samples_per_segment)
    segment <- dplyr::tibble(
      longitude = geolocation$longitude[[i]] + fraction * (geolocation$longitude[[i + 1]] - geolocation$longitude[[i]]),
      latitude = geolocation$latitude[[i]] + fraction * (geolocation$latitude[[i + 1]] - geolocation$latitude[[i]])
    )
    out[[i]] <- segment
  }
  dplyr::bind_rows(out)
...
get_ereefs_profile <- function(var_names = c("Chl_a_sum", "TN"),
  ...
  if (is.null(grids$z_grid)) {
    stop("z_grid is required for profile extraction but was not found.")
  }
  location <- if (is.integer(geolocation)) {
    dplyr::filter(grids$spatial_grid, i == geolocation[[1]], j == geolocation[[2]])
  } else {
    ereefs_nearest_cells(
      dplyr::tibble(latitude = geolocation[[1]], longitude = geolocation[[2]]),
      grids$spatial_grid
    )
  }
  ...
  botz_scalar <- ereefs_z_sign(input_file, override_positive = override_positive) *
    as.numeric(ereefs_read_var_array(
      ...
      filters = {
        ...
        if (!is.na(botz_j_name)) {
          botz_filters[[botz_j_name]] <- location$j[[1]]
        }
        botz_filters
      }
    ))
  ...
  wet_tbl <- ereefs_wet_layer_table(
    botz_tbl = dplyr::tibble(row_id = 1L, i = location$i[[1]], j = location$j[[1]], botz = botz_scalar),
    eta_tbl = matched_point %>%
      dplyr::select(row_id, i, j) %>%
      dplyr::slice(rep(1L, length(dates))) %>%
      dplyr::mutate(time = dates, eta = eta_record),
    z_grid = grids$z_grid
  )
  wet_surface_tbl <- ereefs_surface_bottom_k(wet_tbl, which_layer = "surface") %>%
    dplyr::arrange(.data$time)

  for (time_index in seq_along(dates)) {
    wet_k <- wet_surface_tbl$k[[time_index]]
    for (var_index in seq_along(var_names)) {
      values[, var_index, time_index] <- ereefs_promote_surface_fill_vector(
        values[, var_index, time_index],
        wet_k = wet_k
      )
    }
  }

  if (squeeze && dim(values)[[3]] == 1) {
    values <- array(values, dim = dim(values)[c(1, 2)])
    colnames(values) <- var_names
  }
  if (squeeze && length(var_names) == 1 && length(dim(values)) == 3) {
    values <- array(values, dim = dim(values)[c(1, 3)])
  }
...
get_ereefs_slice <- function(var_names = c("Chl_a_sum", "TN"),
  ...
  if (nrow(matched) == 0) {
    stop("The line segment given does not intersect any model cells on this grid.")
  }
  ...
  if (robust) {
    ...
    z_grid <- profiles[[1]]$z_grid
    eta <- vapply(profiles, function(x) as.numeric(x$eta[[1]]), numeric(1))
    botz <- vapply(profiles, function(x) as.numeric(x$botz[[1]]), numeric(1))
    values <- array(
      NA_real_,
      dim = c(length(z_grid) - 1, nrow(matched), length(var_names)),
      dimnames = list(seq_len(length(z_grid) - 1), NULL, var_names)
    )

    for (row_id in seq_along(profiles)) {
      profile_values <- profiles[[row_id]]$profiles
      if (length(dim(profile_values)) == 2) {
        values[, row_id, ] <- profile_values
      } else {
        values[, row_id, ] <- profile_values[, , 1]
      }
    }
  } else {
    file_to_use <- ereefs_pick_file_for_date(resolved_files, target_date)
    timing <- get_origin_and_times(file_to_use)
    time_index <- ereefs_time_indices(
      timing[[2]],
      target_date,
      target_date,
      nearest_if_empty = TRUE
    )
    if (!length(time_index)) {
    ...
    values <- array(
      NA_real_,
      dim = c(n_layers, nrow(matched), length(var_names)),
      dimnames = list(seq_len(n_layers), NULL, var_names)
    )

    botz_tbl <- ereefs_botz_table(file_to_use, matched, override_positive = override_positive)
    eta_tbl <- ereefs_eta_table(
      ...
      dplyr::filter(.data$time == selected_time)
    wet_tbl <- ereefs_wet_layer_table(botz_tbl, eta_tbl, z_grid) %>%
      dplyr::filter(.data$time == selected_time) %>%
      dplyr::arrange(.data$row_id, .data$k)

    i_vals <- seq(min(matched$i), max(matched$i))
    j_vals <- seq(min(matched$j), max(matched$j))
    i_lookup <- setNames(seq_along(i_vals), i_vals)
    j_lookup <- setNames(seq_along(j_vals), j_vals)
    ...
    for (var_index in seq_along(var_names)) {
      ...
      arr <- array(arr, dim = c(length(i_vals), length(j_vals), n_layers, length(time_index)))

    for (row_id in seq_len(nrow(matched))) {
      i_rel <- i_lookup[[as.character(matched$i[[row_id]])]]
      j_rel <- j_lookup[[as.character(matched$j[[row_id]])]]
      values[, row_id, var_index] <- arr[i_rel, j_rel, , time_pos]
    }

      surface_wet_tbl <- ereefs_surface_bottom_k(wet_tbl, which_layer = "surface")
      for (row_id in seq_len(nrow(matched))) {
        wet_match <- surface_wet_tbl$k[surface_wet_tbl$row_id == row_id]
        if (!length(wet_match) || is.na(wet_match[[1]])) {
          next
        }
        wet_k <- wet_match[[1]]
        values[, row_id, var_index] <- ereefs_promote_surface_fill_vector(
          values[, row_id, var_index],
          wet_k = wet_k
        )
      }
    }
    ...
    dry_layers <- wet_tbl %>%
      dplyr::mutate(is_dry = .data$dz <= 0) %>%
      dplyr::select(.data$row_id, .data$k, .data$is_dry)
    for (row_id in seq_len(nrow(matched))) {
      dry_k <- dry_layers$k[dry_layers$row_id == row_id & dry_layers$is_dry]
      if (length(dry_k)) {
        values[dry_k, row_id, ] <- NA_real_
      }
    }
...
ereefs_slice_distance_bounds <- function(slice) {
  centres <- dplyr::as_tibble(slice$cell_centres)
  n_cells <- nrow(centres)
  if (n_cells == 0) {
    stop("Slice does not contain any intersected cells.")
  }
  if (n_cells == 1) {
    return(dplyr::tibble(xmin = 0, xmax = 0))
  }

  if (!is.null(slice$cell_intersections) && nrow(slice$cell_intersections) == (n_cells + 1L)) {
    segment_lengths <- vapply(
      seq_len(n_cells),
      function(i) {
        earth.dist(
          slice$cell_intersections[i, "longitude"],
          slice$cell_intersections[i, "latitude"],
          slice$cell_intersections[i + 1L, "longitude"],
          slice$cell_intersections[i + 1L, "latitude"]
        )
      },
      numeric(1)
    )
    edges <- c(0, cumsum(segment_lengths))
    return(dplyr::tibble(xmin = edges[1:n_cells], xmax = edges[2:(n_cells + 1L)]))
  }
  ...
  centre_steps <- vapply(
    ...
    function(i) {
      earth.dist(
        centres$longitude[[i]],
        centres$latitude[[i]],
        centres$longitude[[i + 1L]],
        centres$latitude[[i + 1L]]
      )
    },
    numeric(1)
  )
  centre_positions <- c(0, cumsum(centre_steps))
  edges <- c(
    0,
    (centre_positions[-1L] + centre_positions[-length(centre_positions)]) / 2,
    centre_positions[[length(centre_positions)]]
  )
  dplyr::tibble(xmin = edges[1:n_cells], xmax = edges[2:(n_cells + 1L)])
}

ereefs_plot_z_grid <- function(z_grid) {
  plot_z_grid <- as.numeric(z_grid)
  finite_idx <- which(is.finite(plot_z_grid) & abs(plot_z_grid) < 1e10)
  if (!length(finite_idx)) {
    return(plot_z_grid)
  }

  top_idx <- max(finite_idx)
  if (top_idx < length(plot_z_grid)) {
    if (top_idx > 1L) {
      top_step <- plot_z_grid[top_idx] - plot_z_grid[top_idx - 1L]
    } else {
      top_step <- 1
    }
    if (!is.finite(top_step) || top_step <= 0) {
      diffs <- diff(plot_z_grid[finite_idx])
      diffs <- diffs[is.finite(diffs) & diffs > 0]
      top_step <- if (length(diffs)) stats::median(diffs) else 1
    }
    plot_z_grid[(top_idx + 1L):length(plot_z_grid)] <- plot_z_grid[top_idx] + top_step
  }

  plot_z_grid
}

plot_ereefs_slice <- function(slice, var_name = "Chl_a_sum", scale_col = "viridis", scale_lim = NA, var_units = "") {
  numprofiles <- dim(slice$values)[2]
  layers <- length(slice$z_grid) - 1
  plot_z_grid <- ereefs_plot_z_grid(slice$z_grid)
  static_zmin <- array(plot_z_grid[1:layers], c(layers, numprofiles))
  static_zmax <- array(plot_z_grid[2:(layers + 1)], c(layers, numprofiles))
  zmin <- static_zmin
  zmax <- static_zmax
  ...
  x_bounds <- ereefs_slice_distance_bounds(slice)
  dmin <- t(array(x_bounds$xmin, c(numprofiles, layers)))
  dmax <- t(array(x_bounds$xmax, c(numprofiles, layers)))

  values <- slice$values[, , var_name]
  ind <- which(!is.na(values))
  collapsed <- ind[(zmax[ind] - zmin[ind]) <= 0]
  if (length(collapsed)) {
    for (flat_idx in collapsed) {
      layer_idx <- ((flat_idx - 1L) %% layers) + 1L
      profile_idx <- ((flat_idx - 1L) %/% layers) + 1L
      wet_layers <- which(
        slice$z_grid[-length(slice$z_grid)] < slice$eta[[profile_idx]] &
          slice$z_grid[-1L] > slice$botz[[profile_idx]]
      )

      # Simple-format files can carry copied-up surface values in layers that
      # sit fully above the free surface. Draw those values in the shallowest
      # physically wet layer rather than above sea level.
      display_layer <- if (length(wet_layers)) max(wet_layers) else layer_idx
      zmin[[flat_idx]] <- max(static_zmin[[display_layer, profile_idx]], slice$botz[[profile_idx]], na.rm = TRUE)
      zmax[[flat_idx]] <- min(static_zmax[[display_layer, profile_idx]], slice$eta[[profile_idx]], na.rm = TRUE)

      if (!is.finite(zmin[[flat_idx]]) || !is.finite(zmax[[flat_idx]]) || zmax[[flat_idx]] <= zmin[[flat_idx]]) {
        zmin[[flat_idx]] <- max(static_zmin[[display_layer, profile_idx]], slice$botz[[profile_idx]], na.rm = TRUE)
        zmax[[flat_idx]] <- max(static_zmax[[display_layer, profile_idx]], zmin[[flat_idx]], na.rm = TRUE)
      }
    }
  }
  if (length(scale_lim) == 1) {
  ...
  p <- ggplot2::ggplot(data = mydata, ggplot2::aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = z)) +
    ggplot2::geom_rect() +
    ggplot2::ylab("metres above msl") +
    ggplot2::xlab("kilometres along transect")
...
find_intersections <- function(geolocation, x_grid, y_grid, latitude, longitude, first_point = FALSE) {
	a <- (dim(x_grid) - 1)[1]
	b <- (dim(x_grid) - 1)[2]
  ...
  # Rearrange to get an array with one row for each cell and one column for each corner of that cell (top left/bottom left/bottom right/top right)
	...
	lon1 <- geolocation[1,'longitude']
	lon2 <- geolocation[2,'longitude']
	lat1 <- geolocation[1,'latitude']
	lat2 <- geolocation[2,'latitude']
  ...
  #browser()
   geolocation <- data.frame(latitude=latitude[llind], longitude=longitude[llind]) 
   return(list(geolocation, location_edges, intersected, llind))
}
# metadata:
# - gpt_version: GPT-5 Codex
# - time: 00:02
...
# - time: 11:53
# - date: 2026-04-28
# - prompt_used: "Fix the blank live slice vignette example by checking botz sign conventions and making plot_ereefs_slice() fall back to finite model-layer rectangles when non-NA surface values collapse to zero height."
# metadata:
# - gpt_version: GPT-5 Codex
# - time: 12:05
# - date: 2026-04-28
